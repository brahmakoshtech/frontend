...
export default {
  ...
  setup() {
    ...
    const activeTab = ref('system'); // 'system' | 'apis'
    const filterStatus = ref('all'); // 'all' | 'healthy' | 'failed'
    let intervalId = null;
          ...
          {activeTab.value === 'system' && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '1.25rem', marginBottom: '2rem' }}>
                ...
                <MetricCard title="Total Clients" value={d.stats.totalClients} sub={`${d.stats.activeClients} active`} accent="#6366f1" bg="#eef2ff"
                  icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 21h18M5 21V7l8-4v18M19 21V11l-6-4"/></svg>} />
                <MetricCard title="Total Users" value={d.stats.totalUsers} sub={`${d.stats.activeUsers} active`} accent="#0ea5e9" bg="#e0f2fe"
